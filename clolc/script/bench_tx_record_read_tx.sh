...
for i in 1000 10000 100000 1000000; do
    ...
    sleep 5
    ./clolc.out -phase tr -process prepare_tx -numTXs $i | tee -a $LOG_FILE_DIR
    for j in {1..10}; do
        echo "No: $j" >>$LOG_FILE_DIR
        ./clolc.out -phase tr -process read_tx -numTXs $i -numIter 1 | tee -a $LOG_FILE_DIR
        sleep 1
    done
done
...
for i in 1000 10000 100000 1000000; do
    ...
    for j in {1..10}; do
        echo "No: $j" >>$LOG_FILE_DIR
        ./clolc.out -phase tr -process read_all_tx -numTXs $i -numIter 1 | tee -a $LOG_FILE_DIR
        sleep 1
    done
done
