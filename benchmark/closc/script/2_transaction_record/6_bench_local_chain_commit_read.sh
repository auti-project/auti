...
LOG_FILE_DIR="${LOG_DIR}/closc_tr_local_chain_commit_read.log"
...
FABLO_LOCAL_CHAIN_CONFIG="local-chain-commit-config.yaml"
...
for i in 1000 9000 90000 900000; do
  ./closc.out -phase tr -process local_commit_prepare -num $i | tee -a $LOG_FILE_DIR
  TOTAL_TXS=$((TOTAL_TXS + i))
  sleep 5

  for j in {1..10}; do
    echo "No: $j" >>$LOG_FILE_DIR
    ./closc.out -phase tr -process local_commit_read -num $TOTAL_TXS -numIter 1 | tee -a $LOG_FILE_DIR
    sleep 5
    ./closc.out -phase tr -process local_commit_read_all -num $TOTAL_TXS -numIter 1 | tee -a $LOG_FILE_DIR
    sleep 5
  done
