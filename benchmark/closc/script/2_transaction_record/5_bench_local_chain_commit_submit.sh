...
LOG_FILE_DIR="${LOG_DIR}/closc_tr_local_chain_commit_submit.log"
...
sleep 5
./closc.out -phase tr -process local_commit_submit -num 1000 -numIter 10 | tee -a $LOG_FILE_DIR
sleep 5
...
for i in 10000 100000 1000000; do
  for j in {1..10}; do
    ...
    docker ps -a --format '{{.Names}}' | grep '^ca' | xargs docker rm -f
    sleep 5
    ./closc.out -phase tr -process local_commit_submit -num $i -numIter 1 | tee -a $LOG_FILE_DIR
    sleep 5
  done
done
