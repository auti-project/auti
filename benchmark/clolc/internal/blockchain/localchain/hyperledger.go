...
import (
	...
	"github.com/auti-project/auti/benchmark/clolc/internal/constant"
...
func SubmitTX(numTXs int) ([]string, error) {
	...
	var txIDs []string
	startTime := time.Now()
	for batch := 0; batch < numTXs; batch += constant.SubmitTXBatchSize {
		right := batch + constant.SubmitTXBatchSize
		if right > numTXs {
			right = numTXs
		}
		for trial := 0; trial < constant.SubmitTXMaxRetries; trial++ {
...
func ReadTX() error {
	f, err := os.Open(constant.LocalChainTXIDLogPath)
	if err != nil {
		return err
	}
...
func SaveTXIDs(txIDs []string) error {
	f, err := os.OpenFile(constant.LocalChainTXIDLogPath, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
	if err != nil {
		return err
	}
